...
public:
    ...
    int32_t CreateVideoDecoder(const std::string &name);
    int32_t Configure();
    int32_t Prepare();
    int32_t Start();
    int32_t Stop();
    ...
    int32_t ReadData(std::shared_ptr<AVSharedMemory> buffer, uint32_t index);
    int32_t SendData(uint32_t bufferSize, uint32_t index, std::shared_ptr<AVSharedMemory> buffer);
    int32_t StateEOS();
    void CopyStartCode(uint8_t *frameBuffer, uint32_t bufferSize, AVCodecBufferInfo &info, AVCodecBufferFlag &flag);
