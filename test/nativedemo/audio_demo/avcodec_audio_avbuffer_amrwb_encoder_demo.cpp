...
constexpr int64_t BIT_RATE = 15850;
constexpr int32_t SAMPLE_FORMAT = AudioSampleFormat::SAMPLE_S16LE;
constexpr int32_t INPUT_FRAME_BYTES = 640;  // 20ms

constexpr string_view INPUT_FILE_PATH = "/data/test/media/amrwb_16kHz_10s.pcm";
constexpr string_view OUTPUT_FILE_PATH = "/data/test/media/amrwb_16kHz_10s_afterEncode.raw";
}  // namespace
...
void AEncAvbufferAmrWbDemo::RunCase()
{
    std::cout << "amrwb RunCase enter" << std::endl;
...
int32_t AEncAvbufferAmrWbDemo::GetFileSize(const std::string& filePath)
{
...
AEncAvbufferAmrWbDemo::AEncAvbufferAmrWbDemo()
...
int32_t AEncAvbufferAmrWbDemo::CreateEnc()
{
...
int32_t AEncAvbufferAmrWbDemo::Configure(OH_AVFormat *format)
{
    return OH_AudioCodec_Configure(audioEnc_, format);
}

int32_t AEncAvbufferAmrWbDemo::Start()
{
    isRunning_.store(true);

    inputLoop_ = make_unique<thread>(&AEncAvbufferAmrWbDemo::InputFunc, this);
    DEMO_CHECK_AND_RETURN_RET_LOG(inputLoop_ != nullptr, AVCS_ERR_UNKNOWN, "Fatal: No memory");

    outputLoop_ = make_unique<thread>(&AEncAvbufferAmrWbDemo::OutputFunc, this);
    DEMO_CHECK_AND_RETURN_RET_LOG(outputLoop_ != nullptr, AVCS_ERR_UNKNOWN, "Fatal: No memory");

    return OH_AudioCodec_Start(audioEnc_);
}

int32_t AEncAvbufferAmrWbDemo::Stop()
{
...
int32_t AEncAvbufferAmrWbDemo::Flush()
{
    isRunning_.store(false);
    if (inputLoop_ != nullptr && inputLoop_->joinable()) {
        {
...
int32_t AEncAvbufferAmrWbDemo::Reset()
{
    return OH_AudioCodec_Reset(audioEnc_);
}

int32_t AEncAvbufferAmrWbDemo::Release()
{
    return OH_AudioCodec_Destroy(audioEnc_);
}

void AEncAvbufferAmrWbDemo::HandleEOS(const uint32_t& index)
{
...
void AEncAvbufferAmrWbDemo::InputFunc()
{
...
void AEncAvbufferAmrWbDemo::OutputFunc()
{
