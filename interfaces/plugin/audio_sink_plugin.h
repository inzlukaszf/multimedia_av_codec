...
struct AudioSinkPlugin : public Plugins::PluginBase {
    ...
    /**
     * @brief Pauses audio rendering without clear device lists
     *
     * The function is valid only in the RUNNING state. If the pause is successful,
     * the plugin enters the PAUSED state.
     *
     ...
     * @retval OK: Plugin Pause succeeded.
     */
    virtual Status PauseTransitent() = 0;
     ...
     */
    virtual Status Write(const std::shared_ptr<AVBuffer>& input) = 0;
 ...
 * @version 1.0
 */
struct AudioSinkPluginDef : public Plugins::PluginDefBase {
    AudioSinkPluginDef()
    {
        apiVersion = AUDIO_SINK_API_VERSION; ///< Audio sink plugin version.
        pluginType = Plugins::PluginType::AUDIO_SINK; ///< Plugin type, MUST be AUDIO_SINK.
    }
};
} // namespace Plugin
} // namespace Media
